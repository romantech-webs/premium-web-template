    ...
    <section id="opiniones" className="section-padding bg-white relative overflow-hidden">
      ...
      <div className="container-wide relative">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-display font-bold text-secondary mt-4 mb-6 lg:mb-8">
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-4 sm:gap-6 bg-gradient-to-r from-neutral to-white rounded-2xl px-5 sm:px-8 py-4 sm:py-5 shadow-lg shadow-black/5 border border-gray-100"
          >
            <div className="text-center">
              <p
                className="text-4xl sm:text-5xl font-bold text-secondary"
                style={{ textShadow: "0 0 40px color-mix(in srgb, var(--color-primary) 30%, transparent)" }}
              >
                {clinic.reviews.rating}
              </p>
              <div className="flex gap-1 mt-2 justify-center">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className="w-4 sm:w-5 h-4 sm:h-5 text-amber-400" fill="currentColor" />
                ))}
              </div>
            </div>
            <div className="w-px h-12 sm:h-16 bg-gray-200" />
            <div className="text-left">
              <p className="text-2xl sm:text-3xl font-bold text-secondary">{clinic.reviews.count}</p>
              <p className="text-xs sm:text-sm text-secondary/50 uppercase tracking-wider">Reseñas en Google</p>
            </div>
          </motion.div>
        </motion.div>
        ...
        <div className="md:hidden -mx-4">
                ...
                <div className="h-full p-5 bg-gradient-to-br from-white to-neutral rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden">
                  <div className="flex gap-1 mb-3">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className={cn("w-4 h-4", i < review.rating ? "text-amber-400" : "text-gray-200")}
                        fill="currentColor"
                      />
                    ))}
                  </div>
          ...
          <div className="flex justify-center gap-2 mt-4">
            {clinic.reviews.featured.map((_, i) => (
              <button
                key={i}
              ...
              >
                <div className={cn(
                  ...
                  <div className={cn(
                    ...
                    <Quote className={cn("text-primary", isFeatured ? "w-6 h-6" : "w-5 h-5")} />
                  </div>
                  ...
                  <p className={cn("text-secondary/70 leading-relaxed mb-6", isFeatured && "text-lg")}>
                    &ldquo;{review.text}&rdquo;
                  </p>
